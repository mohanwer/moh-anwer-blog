...
export default function AuthorLayout({
  ...
  frontMatter,
  path,
}: PropsWithChildren<{ frontMatter: AuthorFrontMatter; path: string }>) {
  const { name, avatar, occupation, company, email, linkedin, github } = frontMatter

  return (
    <>
      <PageSEO title={`About - ${name}`} description={`About me - ${name}`} path={path} />
      <div className="divide-y divide-gray-200 dark:divide-gray-700">
        ...
        <div className="items-start space-y-2 xl:grid xl:grid-cols-3 xl:gap-x-8 xl:space-y-0">
          ...
          <div className="prose dark:prose-dark max-w-none pb-8 pt-8 xl:col-span-2">{children}</div>
        </div>
      </div>
    </>
  )
