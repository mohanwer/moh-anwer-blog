...
export default function ListLayout({
  ...
  useEffect(() => {
    if (searchValue === '') {
      setDisplayPosts(initialDisplayPosts)
      return
    }
        ...
        <ul>
          {displayPosts &&
            displayPosts.map((frontMatter) => {
              const { slug, url, date, title: frontMatterTitle, summary, tags } = frontMatter
              return (
                    ...
                    <div className="space-y-3 xl:col-span-3">
                      <div>
                        <h3 className="text-2xl font-bold leading-8 tracking-tight">
                          <Link href={`/blog/${url}`} className="text-gray-900 dark:text-gray-100">
                            {frontMatterTitle}
                          </Link>
                        </h3>
